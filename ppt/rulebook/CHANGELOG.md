...
경영관리 MVP 4차 — 초보자 마찰 감소·차트 품질. MVP 4개 마일스톤 완료.

- 문체 교정 배선: 죽은 파일이던 `prompts/headline-rewrite.md`를 PROMPTS에 편입({{RULES.STYLE}}을 style.json에서 결정적 조립). 덱보드 체인 줄에 lint 위반 시 "교정안" 버튼 — 대안 2개를 받아 **교정안도 같은 lint로 재검증 후** 제시, 선택 시 기존 head 수정 경로(form_ok→msg_ok 강등 자동). 자동 일괄 적용은 없음 — 선택은 사람이
- §6 차트 규정 보강: 범례 형태 명문화(우상단 스와치+라벨, 계열 3개 이상 자동 — 실물 백포트), 강조 박스 형태(problem색 점선 라운드)
- 엔진 백포트(실물 승인 덱 유래, 전부 옵트인 — 골든 불변): bars emphasisBox 렌더 구현(선언만 있던 실갭)·범례 스와치(`legend`, 계열 3+ 자동)·전 계열 값 라벨(`dataLabels`), trend emphasisBox(구간 점선 박스)
- showcase에 옵션 시연 장(s9 bars 3계열) 추가, artifact:test 74건

## 도구 추가 (2026-08-14, 규칙 무변경 — v0.2.2 선례)

경영관리 MVP 3차 — 부서 이동 대응 자산. rules는 그대로이고 도구·문서만 추가.

- `engine/pack-probe.py` (`npm run pack:probe`): 벤치마크 pptx에서 판형(sldSz)·색 히스토그램(채움/글자/선/형광 문맥 분리)·폰트·종결어미·기호·영문 라벨 후보를 추출 — 조직 팩 초안의 재료. 리허설: 골든·실물 덱에 실행해 기지 사실(13.33×7.5, 맑은 고딕, 역할색 hex, 대시 부재) 재현 확인
- `prompts/pack-draft.md`: probe 산출 → org 팩 4파일 초안을 만드는 Claude Code 지시문 정본 (판정 원칙 6개 + PDF 대체 절차. 자동 생성이 아니라 초안+사람 승인 — 종결·금칙은 해석의 영역)
- README 부서 이동 절차 개정: 1단계 probe 도구화, 5단계에 showcase 재빌드 추가 (빌드 성공 = 새 팩 role 키 완전성 증명)
- `library/` 신설: 승인 확정 덱 동결 규약 + INDEX (폼 스터디 승률과 층위 구분 — 자동 연동 없음)
